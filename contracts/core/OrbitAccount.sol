...
contract OrbitAccount is ReentrancyGuard {
     ...
     * @dev FIXED: Now transfers actual balance received, not vault's calculated amount
     */
    function withdraw(
        uint256 amount,
        address asset
        ...
        // Calculate shares to redeem proportionally - SAFE VERSION
        uint256 currentDeposit = deposits[asset];
        require(currentDeposit > 0, "No deposits");

        uint256 sharesToRedeem;
        // If withdrawing everything or more than available, redeem all shares
        if (amount >= currentDeposit) {
            sharesToRedeem = totalShares;
        } else {
            // Partial withdrawal - calculate proportional shares
            sharesToRedeem = (totalShares * amount) / currentDeposit;
        }

        // CRITICAL FIX: Get balance before redemption to calculate actual received
        uint256 balanceBefore = IERC20(asset).balanceOf(address(this));

        // Redeem from vault - this brings tokens into this contract
        ...
        // CRITICAL FIX: Calculate actual tokens received
        // The vault may return a calculated amount with yield, but strategy only transfers what it has
        uint256 balanceAfter = IERC20(asset).balanceOf(address(this));
        uint256 actualReceived = balanceAfter - balanceBefore;

        // Update deposits and shares BEFORE transfer
        deposits[asset] -= amount;
        strategyShares[asset] -= sharesToRedeem;

        // CRITICAL FIX: Transfer only the actual balance received, not the vault's calculated amount
        IERC20(asset).safeTransfer(msg.sender, actualReceived);

        // Remove asset from tracking if fully withdrawn
        if (deposits[asset] == 0 && isDepositedAsset[asset]) {
            _removeDepositedAsset(asset);
        }
        ...
        emit Withdrawal(asset, actualReceived, deposits[asset]);
    }
    ...
    function liquidate(
        ...
        uint256 sharesToRedeem = (totalShares * requiredCollateral) /
            totalDepositAmount;
    ...
    function _calculateMaxBorrowable(
        ...
        for (uint256 i = 0; i < assets.length; i++) {
            ...
            // CRITICAL FIX: Normalize decimals for assets like USDC (6 decimals)
            uint256 normalizedAmount = depositAmount;
            uint8 assetDecimals = IERC20Metadata(asset).decimals();
            if (assetDecimals < 18) {
                normalizedAmount = depositAmount * (10 ** (18 - assetDecimals));
            }

            // Calculate asset value in USD using normalized amount
            uint256 assetValue = (normalizedAmount * assetPrice) / 1e18;
     ...
     * @dev CRITICAL FIX: Now handles multi-asset scenarios by calculating proportional debt
     */
    function _calculateWithdrawable(
        ...
        // Normalize this asset's deposit amount
        uint256 normalizedDeposit = deposits[asset];
        uint8 assetDecimals = IERC20Metadata(asset).decimals();
        if (assetDecimals < 18) {
            normalizedDeposit = deposits[asset] * (10 ** (18 - assetDecimals));
        }

        // Calculate this asset's value in USD
        uint256 thisAssetValue = (normalizedDeposit * assetPrice) / 1e18;

        // Get total collateral value across ALL assets
        uint256 totalCollateralValue = 0;
        address[] memory assets = _getDepositedAssets();
        for (uint256 i = 0; i < assets.length; i++) {
            address currentAsset = assets[i];
            uint256 depositAmount = deposits[currentAsset];
            
            if (depositAmount == 0) continue;
            
            uint256 currentAssetPrice = oracle.getPrice(currentAsset);
            
            // Normalize current asset
            uint256 normalizedCurrentDeposit = depositAmount;
            uint8 currentAssetDecimals = IERC20Metadata(currentAsset).decimals();
            if (currentAssetDecimals < 18) {
                normalizedCurrentDeposit = depositAmount * (10 ** (18 - currentAssetDecimals));
            }
            
            uint256 currentAssetValue = (normalizedCurrentDeposit * currentAssetPrice) / 1e18;
            totalCollateralValue += currentAssetValue;
        }
        ...
        // Calculate this asset's proportional share of the total debt
        // This is the key fix: each asset only needs to cover its share of the debt
        uint256 debtShareForThisAsset = (totalDebt * thisAssetValue) / totalCollateralValue;

        // Now calculate withdrawable using ONLY this asset's debt share
        uint256 normalizedWithdrawable = AccountingLib.calculateWithdrawable(
            normalizedDeposit,
            debtShareForThisAsset,  // ← Only this asset's proportional debt!
            assetPrice,
            debtTokenPrice,
            ltv
        );
        ...
        if (assetDecimals < 18) {
            return normalizedWithdrawable / (10 ** (18 - assetDecimals));
        }

        return normalizedWithdrawable;
    }
    ...
    function _removeDepositedAsset(address asset) internal {
        ...
        for (uint256 i = 0; i < depositedAssets.length; i++) {
            if (depositedAssets[i] == asset) {
                ...
                depositedAssets[i] = depositedAssets[
                    depositedAssets.length - 1
                ];
                depositedAssets.pop();
